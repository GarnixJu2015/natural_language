...
def search_lemma(word):
    with get_connection() as conn:
		cur = conn.cursor()
		cmd = 'SELECT lemma FROM WordLemma WHERE word="%s" ORDER BY probability DESC;' % (word)
...
def search_tag(word):
    with get_connection() as conn:
        ...
        # for res in cur.execute(cmd):
        #     return res[1]        
        cmd = 'SELECT tag, MAX(probability) FROM WordLemma WHERE word="%s" ' % (word)
        for res in cur.execute(cmd):
            return res[0]
